...
class RiskController extends Controller {
    ...
    public function createAction() {
        if ($this->requestType() == "GET") {
            ...
            $this->risk->setRiskHazards($this->requestObject(), $riskId);
...
//        $this->word->setDefaultFontName('Helvetica');
        ...
        $this->header($section, str_replace(" ", "", $riskAssessment['name']));
        $this->footer($section);
        ...
        foreach ($hazards as $hazard) {
            ...
            $hazCell = $hazardTable->addCell(2200);
            $hazCell->getStyle()->setVAlign("center");
            $hazCell->addText(htmlspecialchars($hazard['name']), $pBold, $center);
            $consecell = $hazardTable->addCell(2500);
            \PhpOffice\PhpWord\Shared\Html::addHtml($consecell, str_replace(array("&"), "and", trim(preg_replace('/\t+/', '', $hazard['description']))));

            $likeCell = $hazardTable->addCell(1500);
            $likeCell->getStyle()->setVAlign("center");
            $likeCell->addText($hazard['likelihood'], $pBold, $center);

            $sevCell = $hazardTable->addCell(1500);
            $sevCell->getStyle()->setVAlign("center");
            $sevCell->addText($hazard['severity'], $pBold, $center);

            $riskRankCell = $hazardTable->addCell(1500);
    ...
    public function documentActionold($id) {
